...
def op_checksig(stack, z):
    ...
    sec_pubkey_obj = S256Point.parse(sec_pubkey)
    der_sig_obj = Signature.parse(der_sig)
    # verify the signature using S256Point.verify()
    # push an encoded 1 or 0 depending on whether the signature verified
    if sec_pubkey_obj.verify(z, der_sig_obj): 
        # if the signature is valid, push b'\x01' to the stack
        stack.append(encode_num(1))
    else:
        # if the signature is invalid, push b'\x00' to the stack
        stack.append(encode_num(0))
    return True
