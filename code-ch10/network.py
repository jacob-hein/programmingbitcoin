...
class NetworkEnvelope:
    ...
    def parse(cls, s, testnet=False):
        ...
        command = command.strip(b'\x00')  # strip trailing zeros
        print(command)
        ...
        # verify checksum
        payload_checksum_computed = hash256(payload)[:4]
        if payload_checksum != payload_checksum_computed:
...
class VersionMessage:
    ...
    def serialize(self):
        ...
        s += self.receiver_port.to_bytes(2, byteorder='big')
        ...
        s += self.sender_port.to_bytes(2, byteorder='big')
